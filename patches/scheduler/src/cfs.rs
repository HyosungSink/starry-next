...
impl<T> BaseScheduler for CFScheduler<T> {
    ...
    fn remove_task(&mut self, task: &Self::SchedItem) -> Option<Self::SchedItem> {
        let key = (task.get_vruntime(), task.get_id());
        if let Some((_, tmp)) = self.ready_queue.remove_entry(&key) {
            if let Some(((min_vruntime, _), _)) = self.ready_queue.first_key_value() {
                self.min_vruntime = Some(AtomicIsize::new(*min_vruntime));
            } else {
                self.min_vruntime = None;
    ...
    fn put_prev_task(&mut self, prev: Self::SchedItem, preempt: bool) {
        let taskid = prev.get_id();
        let mut vruntime = prev.get_vruntime();
        if !preempt {
            if let Some(((min_ready_vruntime, _), _)) = self.ready_queue.first_key_value() {
                let yielded_vruntime = (*min_ready_vruntime).saturating_add(1);
                if vruntime <= yielded_vruntime {
                    prev.set_vruntime(yielded_vruntime);
                    vruntime = yielded_vruntime;
                }
            }
        }
        self.ready_queue.insert((vruntime, taskid), prev);
        if let Some(((min_vruntime, _), _)) = self.ready_queue.first_key_value() {
            self.min_vruntime = Some(AtomicIsize::new(*min_vruntime));
        } else {
            self.min_vruntime = None;
