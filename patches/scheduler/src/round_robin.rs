...
impl<T, const S: usize> RRTask<T, S> {
    ...
    pub const fn new(inner: T) -> Self {
        Self {
            ...
            base_time_slice: AtomicIsize::new(S as isize),
    ...
    fn reset_time_slice(&self) {
        self.time_slice
            .store(self.base_time_slice(), Ordering::Release);
    }
    ...
    fn base_time_slice(&self) -> isize {
        self.base_time_slice.load(Ordering::Acquire).max(1)
    }

    pub fn set_time_slice_value(&self, time_slice: isize) {
        let clamped = time_slice.max(1);
        self.base_time_slice.store(clamped, Ordering::Release);
        self.time_slice.store(clamped, Ordering::Release);
    }
